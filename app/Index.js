...
Log_1.logger.info("framework run mode: ", Context_1.config.runContext.mode);
if (Context_1.config.runContext.mode === IConfig_1.RunMode.full) {
...
if (Context_1.config.runContext.mode === IConfig_1.RunMode.generateLoad) {
    Log_1.logger.info('start running tab test');
    var executor = new TestExecutor_1["default"]();
    executor.runPerfTest().then(function () {
        Log_1.logger.info('start result processing');
        var resultProcessor = new ResultProcessor_1["default"]();
        return resultProcessor.ProcessResults();
    }).then(function () {
